...
import { TLocationState, TWSOrder } from "../../types/types";
...
export const OrderBlock: FC<IOrderBlock> = ({element, toggleFeedOverlay, pathname, width, height, status}) => {
  let orderPrice: number = 0;
  const needStatus: boolean = status
  const dispatch = useDispatch();
  let location = useLocation<TLocationState>();
  const { standartIngredients,} = useSelector(state => state.ingredients);
  const selectOrder = (e: SyntheticEvent) => {
    e.stopPropagation();
    toggleFeedOverlay()
              ...
              if (standartIngredient._id === ingredient){
                ...
                if (index === 5) {
                  ...
                  <div key={uuidv4()} className={`${styles['pic-ingredients']}`}>
                    ...
                    style={{zIndex: (index+2)*-1, left: index*20*-1 }} 
